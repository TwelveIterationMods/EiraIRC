...
public class GuiBotCommands extends GuiScreen {
	...
	public void initGui() {
		...
		txtDisabledInterOpCommands = new GuiAdvancedTextField(fontRenderer, rightX, topY + 2, BUTTON_WIDTH, 16);
		txtDisabledInterOpCommands.setEnabled(false);
		txtDisabledCommands = new GuiAdvancedTextField(fontRenderer, rightX, topY + 42, BUTTON_WIDTH, 16);
	...
	private void loadFromProfile() {
		...
		for(IBotCommand command : botProfile.getCommands()) {
			if(command instanceof BotCommandCustom) {
				...
				listCommands.addEntry(new GuiListTextEntry(fontRenderer, command.getCommandName(), listCommands.getEntryHeight(), Globals.TEXT_COLOR) {
					@Override
					public void setSelected(boolean selected) {
						Minecraft.getMinecraft().displayGuiScreen(new GuiBotCommand(GuiBotCommands.this, botProfile, customCommand));
						Minecraft.getMinecraft().sndManager.playSoundFX("gui.button.press", 1f, 1f);
					}
				});
			}
		}
		listCommands.addEntry(new GuiListTextEntry(fontRenderer, "Add...", listCommands.getEntryHeight(), -16711936) {
			@Override
			public void setSelected(boolean selected) {
	...
	public void drawScreen(int par1, int par2, float par3) {
		drawBackground(0);
		this.drawCenteredString(fontRenderer, Utils.getLocalizedMessage("irc.gui.botCommands"), width / 2, height / 2 - 110, Globals.TEXT_COLOR);
		drawString(fontRenderer, "Disabled InterOp Commands:", width / 2, height / 2 - 75, Globals.TEXT_COLOR);
		txtDisabledInterOpCommands.drawTextBox();
		drawString(fontRenderer, "Disabled Bot Commands:", width / 2, height / 2 - 34, Globals.TEXT_COLOR);
		txtDisabledCommands.drawTextBox();
		drawString(fontRenderer, "An asterisk ('*') disables all commands.", width / 2, height / 2 + 6, Globals.TEXT_COLOR);
		drawString(fontRenderer, "Custom Bot Commands:", width / 2 - 170, height / 2 - 34, Globals.TEXT_COLOR);
