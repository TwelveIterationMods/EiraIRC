...
public class OverlayRecLive extends Gui {
	...
	public void updateAndRender(float delta) {
		...
		NetClientHandler handler = theGame.thePlayer.sendQueue;
		if (theGame.gameSettings.keyBindPlayerList.pressed && (!theGame.isIntegratedServerRunning() || handler.playerInfoList.size() > 1 || scoreobjective != null)) {
			...
			for (int i = 0; i < maxPlayers; i++) {
				...
				if (i < players.size()) {
					...
					if(playerInfo.isLive) {
						...
						drawString(theGame.field_146289_q, LIVE, xPos + columnWidth - 12 - liveWidth - 2, yPos, Globals.TEXT_COLOR);
		...
		if (DisplayConfig.hudRecState) {
			...
			if (playerInfo.isLive) {
				...
				drawString(theGame.field_146289_q, LIVE, windowWidth - liveWidth - MARGIN, MARGIN + theGame.field_146289_q.FONT_HEIGHT / 2, Globals.TEXT_COLOR);
			} else if (playerInfo.isRecording) {
